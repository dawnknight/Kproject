...
def rel_kin(joints): # kinematic term    
    ...
    for i in xrange(len(order1)):
        ...
        tmp = min(np.abs(np.linalg.norm(A-B) - kinseg[refer1[i]])/kinseg[refer1[i]],1)
        segrel[order1[i]] += tmp
        segrel[order2[i]] += tmp
        

        tmp = min(np.abs(np.linalg.norm(A-C) - kinseg[refer2[i]])/kinseg[refer2[i]],1)
        segrel[order1[i]] += tmp
        segrel[order3[i]] += tmp
...
def rel_rate(Rb,Rk,Rt,flen = 6):
    if (len(Rb)>=flen) & (len(Rk)>=flen) & (len(Rt)>=flen) :
        rel = 0 #reliability
        for i in xrange(flen):
            rel += gw(i)*min(Rb[-(i+1)],Rk[-(i+1)],Rt[-(i+1)])
    else:
        return 0
